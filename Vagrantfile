...
Vagrant.configure(2) do |config|
  ...
  config.vm.define "vi-nation", primary: true do |vi|
    # You can search for boxes at https://atlas.hashicorp.com/search
    vi.vm.box = "ole/jessie64"
    vi.vm.box_version = "0.1.6"
    vi.vm.hostname = "vi"
    
    vi.vm.provider "virtualbox" do |vb|
      # vb.gui = true
      vb.name = "vi"
      vb.memory = "666"
    end
    ...
    vi.vm.network "forwarded_port", guest: 5984, host: 5985, host_ip: "127.0.0.1", auto_correct: true
    vi.vm.network "forwarded_port", guest: 8080, host: 8085, host_ip: "127.0.0.1", auto_correct: true
    vi.vm.network "forwarded_port", guest: 22, host: 2222, host_ip: "127.0.0.1", id: "ssh", auto_correct: true

    vi.vm.provision "shell", inline: <<-SHELL
  ...
  config.vm.define "vi-community", autostart: false do |community|
    # You can search for boxes at https://atlas.hashicorp.com/search
    community.vm.box = "ole/jessie64"
    community.vm.box_version = "0.1.6"
    community.vm.hostname = "community"
    
    community.vm.provider "virtualbox" do |vb|
      # vb.gui = true
      vb.name = "community"
      vb.memory = "666"
    end
    ...
    # accessing "localhost:8080" will access port 80 on the guest machine.
    # config.vm.network "forwarded_port", guest: 80, host: 8080
    community.vm.network "forwarded_port", guest: 5984, host: 5984, auto_correct: true
    community.vm.network "forwarded_port", guest: 8080, host: 8084, auto_correct: true
    community.vm.network "forwarded_port", guest: 22, host: 2222, host_ip: "0.0.0.0", id: "ssh", auto_correct: true

    # Enable provisioning with a shell script. Additional provisioners such as
    # Puppet, Chef, Ansible, Salt, and Docker are also available. Please see the
    # documentation for more information about their specific syntax and use.
    community.vm.provision "shell", inline: <<-SHELL
      sudo docker run -d -p 5984:5984 --name bell -v /srv/data/bell:/usr/local/var/lib/couchdb -v /srv/log/bell:/usr/local/var/log/couchdb klaemo/couchdb
      # use crontab to start couchdb on boot
      sudo crontab -l | sudo tee -a mycron
      echo "@reboot sudo docker start bell" | sudo tee -a mycron
